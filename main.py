...
async def sanitize_prompt_for_imagen(prompt: str) -> str:
    ...
    1단계: 사전 필터링으로 알려진 민감 단어 치환
    2단계: Gemini를 사용하여 지능적으로 프롬프트를 재작성
    ...
    """
    try:
        # 1단계: 사전 필터링 (알려진 민감 단어 먼저 치환)
        pre_filtered_prompt = pre_filter_sensitive_words(prompt)
        logger.info(f"🔒 1단계 사전 필터링 완료")

        # 2단계: Gemini를 사용한 지능적 재작성
...
ORIGINAL PROMPT: {pre_filtered_prompt}
...
TRANSFORMED PROMPT:"""
        ...
        logger.info("🔄 2단계 Gemini 프롬프트 정제 완료")
        logger.info(f"   원본: {prompt[:100]}...")
        logger.info(f"   사전필터: {pre_filtered_prompt[:100]}...")
        logger.info(f"   최종: {sanitized[:100]}...")

        return sanitized

    except Exception as e:
        logger.warning(f"⚠️ Gemini 프롬프트 정제 중 오류 발생: {e}")
        # 오류 발생 시에도 사전 필터링은 적용된 버전 사용
        try:
            pre_filtered = pre_filter_sensitive_words(prompt)
            logger.info("   사전 필터링만 적용된 프롬프트를 사용합니다.")
            return pre_filtered
        except Exception:
